...
onMounted(() => {
  ...
  setTimeout(() => {
    ...
    if (token) {
      if (role === "admin") {
        showMainAdm.value = true
      } else if (role === "user") {
        showMainUser.value = true
      }
    }
  }, 2000)
    ...
    <transition name="fade" mode="out-in">
      ...
      <div v-else key="main" class="main-content">
        <MainAdm v-if="showMainAdm" @logout="showMainAdm = false"
          @create-user="showUserCreation = true; showMainAdm = false" />
        <UserCreation v-else-if="showUserCreation"
          @create-user-success="showUserCreation = false; showMainAdm = true" />
        <MainUser v-else-if="showMainUser" @logout="showMainUser = false"
          @goToForm="(ins) => { showMainUser = false; currentInspection = ins; showForm = true }" />
        <Form v-else-if="showForm" :initialInspection="currentInspection"
          @submit="() => { showForm = false; showMainUser = true }"
          @cancel="() => { showForm = false; showMainUser = true }" />
